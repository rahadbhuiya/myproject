...
use App\Models\User;                    // <-- Use User model for admins
use App\Models\Transaction;
use App\Notifications\NewOrderNotification;
use App\Notifications\OrderCompleted;
use App\Services\Web3FormNotifier;
...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            Transaction::create([
                ...
                'currency'    => 'BDT', // you may want to make this dynamic
                'status'      => 'completed',
                'description' => 'Order #' . $order->id . ' for Game UID: ' . $order->game_uid,
            ]);

            // Load relations for the order (used in notifications)
            $order->load(['user', 'game', 'product']);

            // Notify all admins about the new order.
            // Use User model to get admins (assuming `is_admin` column exists)
    ...
    {
        // Optionally send order viewed notification via service here
    ...
    public function markAsComplete($id)
    {
        ...
        if (strtolower($order->status) === 'pending') {
            ...
            // Send order completed notification using the external service
